...
#   3.1.74 -> 4.0.11: 3.1.74 had a codegen bug somewhere in the
...
#     config. The 3.1.x line stopped at 3.1.74 (no 3.1.75-3.1.79 in emsdk's
#     registry), so we jumped to 4.0.x where MEMORY64 + SIDE_MODULE +
#     pthreads is production-ready. 4.0.11 was the floor at jump time and
#     happened to already be installed locally.
# If KTX or any other dep regresses at a higher emsdk, step up incrementally
# (4.0.23, etc.) rather than reverting — older toolchains have known
...
export EMSDK_VERSION="${EMSDK_VERSION:-4.0.11}"
