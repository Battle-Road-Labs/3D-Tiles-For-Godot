 ...
 *   <emsdk>/upstream/emscripten/system/lib/compiler-rt/emscripten_setjmp.c
 *
 * Original copyright header preserved below. This file is compiled into
 * the Cesium Godot extension's wasm SIDE_MODULE so that saveSetjmp /
 * testSetjmp / __wasm_longjmp become *defined* symbols inside our own
 * wasm module — resolving undefined-import errors when libjpeg-turbo
 * (compiled with emcc 3.1.56's buggy -sSUPPORT_LONGJMP=wasm lowering)
 * calls into them from TurboJPEG's format-probe error path. Godot's main
 * wasm doesn't provide these symbols, so without this shim every
 * non-JPEG tile aborts the tile-loading worker thread.
 *
 * We must be compiled with -D__USING_WASM_SJLJ__ so the file exposes
 * __wasm_longjmp (matching what libturbojpeg.a expects via the
 * __c_longjmp wasm-EH tag) rather than emscripten_longjmp (the legacy
 * JS-glue path that would also pull in setThrew / _emscripten_throw_longjmp).
 *
 * See CesiumBuildUtils.py comments and memory/project_emsdk_wasm_longjmp_bug.md
